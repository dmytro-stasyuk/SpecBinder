...
- **Custom resolver injection** — a `Clock` filled by `FixedClockResolver` is opted in with `@JUnitResolved` on the parameter
...
- **Custom types do require `@JUnitResolved`** — without it, the generator does not recognize the parameter as JUnit-injected and falls back to its default behavior (regenerates a fresh step method, treating the user's declaration as an unused overload)
