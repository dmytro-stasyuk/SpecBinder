...
Demonstrates using `@Feature2JUnit` without a path — the processor discovers `.feature` files automatically by convention. Feature files live alongside their marker classes in `src/test/java`.

## What this demonstrates

- `@Feature2JUnit` (no value) uses convention-based discovery
...
When `@Feature2JUnit` has no path value, the processor searches for `.feature` files in the same package directory as the annotated class. All `.feature` files found are processed.
...
src/test/java/
  └── dev/specbinder/examples/commonusecases/colocated/
      ├── ShoppingCart.java        ← marker class (@Feature2JUnit)
...
| `src/test/java/.../ShoppingCart.java` | Marker class with bare `@Feature2JUnit` (no path) |
