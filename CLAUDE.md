...
When updating `CHANGELOG.md`, use the `/changelog` skill — it carries the full conventions (only the `## Unreleased` section is editable, released entries are immutable, and breaking API changes get a `**⚠️ BREAKING:** ` prefix).
