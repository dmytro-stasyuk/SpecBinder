...
- **(experimental)** New opt-in `@Gherkin2JUnitOptions(stripMarkerPatterns = {...})` option that removes HTML-like revision markers such as `<CHANGED BR-123>premium</CHANGED BR-123>` from a spec file before it is turned into test code, keeping the text they wrap. Teams that annotate specs with traceability markup tied to an issue tracker previously found that markup reaching the generated code: it changed step method names, so adding or editing a marker renamed an abstract step method and broke the hand-written test class implementing it, and it emitted unbalanced HTML into JavaDoc. Each configured regular expression matches a single marker, opening or closing, and every match is removed on its own — so an unpaired marker is removed just the same, and a pair may wrap several steps without affecting what sits between them. Markers are removed everywhere they can appear: step text, Feature/Rule/Scenario names, descriptions, doc strings, data tables and `Examples` tables, including header cells, where markup would otherwise corrupt generated field names and test method parameter names. Take care that a pattern is specific enough not to match a Scenario Outline `<placeholder>`, which would silently remove a step's parameter; the default is an empty list, which leaves all markup untouched
- **(experimental)** New opt-in `@Gherkin2JUnitOptions(stripRangePatterns = {...})` option that removes a marked range together with the text it wraps — the companion to `stripMarkerPatterns` for markup recording content retired by an issue, such as `<REMOVED BR-789>legacy discount </REMOVED BR-789>`. Each configured regular expression matches a whole range, opening marker through closing marker, so no separate open/close configuration is needed; with the `(?s)` flag a range may wrap several steps, an entire Scenario, or rows of a data table or `Examples` table. Any line left holding only whitespace once a range is removed is dropped entirely, so removing a table row does not leave a gap that would break the table — note this shifts the source line numbers of everything below the range. Ranges are removed before markers, so content marked as removed stays removed even when a marker pattern would also have matched the range's own markers. A range that takes all of a step's text but leaves its keyword behind fails the build with a message naming the line; the default is an empty list, which leaves all markup untouched
